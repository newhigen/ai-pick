...
- ⚠ **`nw` 는 이번 실행에서 넣은 줄에만 붙인다.** 시작할 때 화면에 남아 있는 것을 **먼저 지운다** —
  `class="it t2 nw"` → `class="it t2"`, `<span class="star">*</span>` → `<span class="star"></span>`.
  그다음 이번에 넣은 줄에만 `nw` 와 `*` 를 붙인다. **안 지우면 계속 쌓여 「새로 들어온 것」이 늘기만 한다.**
  머리말 `<b>새로 들어온 것 N개</b>` 의 N 도 그 수로 맞춘다.
  이 화면은 한 시간마다 도니 「새로 들어온 것」은 **마지막 갱신에서 들어온 것**이고,
  상단바의 갱신 시각과 짝을 이룬다.
